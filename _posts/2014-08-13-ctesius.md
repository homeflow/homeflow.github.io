...
We will explore all of the technologies mentioned above in much more detail later in the Wiki. In the meantime you can head straight over to the section you require using the menu on the right or move on to Getting Started below.
...
Draw a Map is an extension of the leaflet mapping system that the Homeflow platform uses to provide freehand polygon drawing to property maps on the website. If you are building a theme and primarily using staging as a test environment it is suggested that you add Ctesius.addConfig('draw_a_map',true) to _js_events_registers.liquid. If you just wish to send it live, ask a Homeflow team member to set the agency or portals draw a map switch to true.

In principle, draw a map works straight out of the box and immediately will appear on any property maps on the client website, held within it's own, independent toolbar. However, depending on the map container and the div hierarchy, it is quite possible that some custom CSS will have to be implemented.

Notably, the save/cancel buttons associated with the drawing, editing and deleting tools can become misaligned and to remedy this one can just append a style to the relevant classes in main.lcss:

.leaflet-control-title-interior

'Sketch-A-Search' box styles
...
It is possible to extensively retheme draw a map (see: www.leaders.co.uk ) but that is beyond the scope of this documentation.
