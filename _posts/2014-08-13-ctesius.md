...
If the user does not select a particular result from the auto suggest ctesius will send to the user to the page as if they selected
the top result.

Some sites may benefit from having those search results further disambiguated. To enable this add 'disambiguation' to your config.yml enabled
features section:
...
This will cause a search with the query 'Brighton' to append ``#/disambiguate/brighton`` to the search result page. Ctesius will then
perform a place search and render a provided view which can be optionally show to the user. (Remember to wrap your template in raw tags)
...
<script id='disambiguation_template' type='text/liquid'>
   <p>Did you mean:</p>
     {%for place in places limit:3%}<p><a href='{{place.url}}'>{{place.name}}</a></p>{%endfor%}</p>
</script>

<div id='disambiguation_view'>
</div>
{%endraw%}
